 ...
 * }
 *
 * zone.createInstance('new-instance-name', config, callback);
 */
Zone.prototype.createInstance = function(name, config, callback) {
  var self = this;
  ...
  if (body.os) {
    this.gceImages.getLatest(body.os, function(err, image) {
      ...
      self.createInstance(name, body, callback);
    });
    return;
  }
 ...
 *
 * @example
 * zone.getInstances(function(err, instances) {
 ...
 *   if (nextQuery) {
 *     // More results exist.
 *     zone.getInstances(nextQuery, callback);
 *   }
 * }
 *
 * zone.getInstances({
...
Zone.prototype.getInstances = function(options, callback) {
  ...
  this.makeReq_('GET', '/instances', options, null, function(err, resp) {
    ...
    var instances = (resp.items || []).map(function(instance) {
